...
def verify_cognito_token(token, user_pool_id, client_id, token_use):
    ...
    try:
        ...
        if token_use == 'id':
            decoded = jose_jwt.decode(token, public_key, algorithms=['RS256'],audience=client_id, issuer=issuer)
        elif token_use == 'access':
            decoded = jose_jwt.decode(token, public_key, algorithms=['RS256'],issuer=issuer, options={'verify_aud': False})
...
def jwt_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        decoded_token, error = verify_cognito_token(token, user_pool_id, client_id, token_use)
        if error:
